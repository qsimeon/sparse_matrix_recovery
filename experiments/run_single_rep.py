...
def one_repetition(rep_idx, seed, num_sessions, max_timesteps, num_nodes,
    ...
    # Edge detection: use > 0 threshold (natural for non-negative constrained
    # estimates where zero means "no edge").  The raw estimate is dense (most
    # entries > 0 after non-negativity clip) so it will have high recall but
    # lower precision.  Granger refinement zeros non-causal entries, improving
    # precision while potentially reducing recall.
    true_edges = (np.abs(true_W) > 0).astype(float)
    est_edges = (approx_W > 0).astype(float)
    opt_edges = (optim_W > 0).astype(float)
