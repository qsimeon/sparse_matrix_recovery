...
% ──────────────────────────────────────────────────────────────
\begin{frame}{Result: $r = 0.96$ at 66\% Coverage}
...
\begin{column}{0.50\textwidth}
  ...
  \begin{tabular}{>{\bfseries\color{accentblue}}l l}
    Correlation & $r = 0.96$ \\
    Over chance & $85\%$ improvement \\
    Best error & $0.052$ ($N{=}30,T{=}1000$) \\
    Coverage & $66\%$ / session \\
    Sessions & $K{=}50$ \\
  \end{tabular}
  \end{center}
  ...
  \vspace{4pt}
  Granger refinement: \textbf{perfect recall}
  --- every true edge preserved.
\end{column}
\end{columns}
...
\begin{tabular}{lcc}
  ...
  \midrule
  $E_1$: Model mismatch (linear $\approx$ tanh) & $0.073$ & $1\times$ \\
  \rowcolor{accentred!15}
  $E_2$: CPG--state correlation & $0.229$ & $\mathbf{3.1\times}$ \\
  \bottomrule
\end{tabular}
\end{center}
...
\begin{column}{0.54\textwidth}
  \begin{enumerate}
    ...
    \item[\color{accentgreen}\textbf{5.}]
      \textbf{Next:} Model CPG dynamics ($3.1\times$ dominant error)
  \end{enumerate}
\end{column}
