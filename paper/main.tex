...
\emph{(iii) Non-negative weights}: $W_{ij} \geq 0$, applied uniformly to all estimates. This is motivated by EM connectomics measuring synapse \emph{size} but not \emph{sign}, yielding an inherently unsigned weight matrix; in our formulation inhibitory effects arise from the sign of the activity rather than the weight. By Perron--Frobenius, this also simplifies spectral radius control. The method generalizes to mixed-sign weights (Section~\ref{sec:discussion}).
...
\paragraph{Stability and input decomposition.}
We scale $\rho(W) \approx 1.0$ (edge of stability), so combined with 1-Lipschitz $\phi$ the autonomous map $x \mapsto W\phi(x)$ is non-expansive and contracts to the origin ($b_t = 0 \Rightarrow x_t \to 0$): the network falls silent without input.
This motivates a central pattern generator (CPG) component of $b_t$: a subset of neurons possess autonomous oscillatory drive \citep{marder2001central} that sustains activity. The input thus decomposes as $b_t = b_t^{\text{stim}} + b_t^{\text{CPG}}$, with $b_t^{\text{stim}}$ extrinsic i.i.d.\ Gaussian (independent of $x_t$) and $b_t^{\text{CPG}} = \text{cpg}(x_t)$ state-dependent. The extrinsic component satisfies our independence assumption; the CPG component violates it and dominates the estimation error (Section~\ref{sec:discussion}).
CPG identity is \emph{hidden from the estimation pipeline}: the experimenter does not know which neurons are CPGs, and the variance-based detection in Section~\ref{sec:discussion} is post-hoc, not an oracle input.
...
\begin{equation}
    ...
    \label{eq:error}
\end{equation}
For $\phi = \tanh$ and approximately Gaussian states, the Stein--Price identity \citep{price1958useful, stein1981estimation} reduces $e_1$ to $W(D - I)$ with $D = \mathrm{diag}\!\big(\mathbb{E}[\mathrm{sech}^2(x_i)]\big)$ and $0 < D_{ii} < 1$:
\begin{equation}
    \Sigma_{\phi(x),x} = D \, \Sigma_{x,x}.
    \label{eq:price}
\end{equation}
The model-mismatch bound therefore tightens to $\|e_1\|_F \leq \|W\|_F \max_i(1 - D_{ii})$, scaling as $O(\sigma^2)$ for small state variance---substantially tighter than a generic Lipschitz bound (full derivation in Appendix~\ref{sec:app_price}). Both error sources are amplified by $\|\Sigma_{x,x}^{-1}\|_2$, so ill-conditioning from sparse measurement degrades recovery even with perfect model knowledge.
...
More broadly, partial-observability identifiability has been studied formally by \citet{janzing2010causal}; our pooling-then-inverting recipe is a second-order finite-sample analog.
\citet{soudry2015shotgun} introduced the multi-session shotgun sampling design and used EM on a GLM; we target the same observation model but pool covariances and invert globally in closed form.
...
Quantitatively, $\|e_2\|_F$ (input correlation) is ${\sim}3\times$ larger than $\|e_1\|_F$ (model mismatch), making CPG correlation the \emph{dominant} error source, not the linear approximation (Figure~\ref{fig:dynamics}E).
...
\textbf{(D)} Per-node activity variance under autonomous dynamics. CPG nodes (orange-red) have systematically higher variance than non-CPG nodes (teal), providing a detection signal.
\textbf{(E)} Error decomposition: $\|e_1\|_F/N$ (model mismatch) vs.\ $\|e_2\|_F/N$ (CPG--state correlation) vs.\ total estimation error $\|\hat{W} - W\|_F/N$. $e_1$ and $e_2$ are Frobenius norms of the individual error components (Eq.~\ref{eq:error}); the total can be smaller because these components partially cancel across matrix entries. For this representative topology, $e_2$ is ${\sim}2.8\times$ larger than $e_1$ (median across topologies: ${\sim}3\times$), confirming that CPG correlation is the dominant error source.
\textbf{(F)} Scatter plot of true off-diagonal weights $W_{ij}$ (horizontal) vs.\ Granger-refined estimates $\hat{W}_{ij}$ (vertical) for this representative topology. Pearson $r = 0.94$; dashed line: $y=x$. Median raw-estimator correlation across all 10 topologies: $r = 0.90$.}
\label{fig:dynamics}
\end{figure}
...
A surprising finding is that the approximate estimator (using $\Sigma_{x,x}$) outperforms the natural plug-in oracle that inverts $\Sigma_{\phi(x),x}$ with known $\phi$ and $b$.
Intuitively, the row-compression $\Sigma_{\phi(x),x} = D \Sigma_{x,x}$ implied by Stein--Price makes the oracle's covariance worse-conditioned than $\Sigma_{x,x}$, and the resulting variance penalty outweighs the oracle's bias advantage; full bias--variance breakdown in Appendix~\ref{sec:app_oracle}.
E6 confirms this empirically: the approximation matches or outperforms the oracle at all seven stimulation intensities tested, with the largest non-pathological gap ($\sim 2.2\times$) at $\sigma = 0.1$.
...
Observation noise enters as ridge regularization (Eq.~\eqref{eq:estimator}); E8 confirms graceful degradation up to $\sigma_\epsilon = 0.5$ ($\sim 32\%$ error increase).
...
We discuss structural identifiability (the coupon-collector bound on session count $K$) and the gap to practical identifiability (the conditioning of $\Sigma_{x,x}$) in Appendix~\ref{sec:app_ident}.
...
(3)~Our experiments span $N \in \{15, 159, 300\}$, motivated by biologically relevant circuit scales: from the lobster stomatogastric ganglion (${\sim}30$ neurons; \citep{marder2007understanding}) to \textit{C.~elegans} (302 neurons; \citep{white1986structure}). The method itself places no upper bound on $N$; the computational bottleneck is the $O(N^3)$ covariance inversion, for which efficient iterative and sparse solvers exist, and recent advances in AI-discovered linear-algebra algorithms \citep{fawzi2022discovering} suggest this ceiling is not fundamental.
(4)~We assume a fixed weight matrix $W$ shared across recording sessions, which holds on the short timescales typical of optogenetic experiments; biological synapses are plastic on longer timescales, so the recovered $\hat{W}$ should be interpreted as a snapshot of effective connectivity rather than a permanent wiring diagram.
...
The mathematical content of this paper is intentionally simple: pool pairwise covariances across sessions, invert once, refine with a Granger-inspired projection.
What that simplicity buys is a route to weighted connectivity that has been almost entirely absent from connectomics practice---one that uses standard functional imaging, asks no more of the experimenter than randomising the imaging mask between sessions, and recovers the full $N{\times}N$ synaptic matrix despite never observing the whole circuit at once.
The empirical claims are correspondingly simple: at the scales we test, this works.

Three findings refine this picture.
First, modelling the network's nonlinearity does not help: a linear estimator outperforms a natural plug-in oracle that knows the true $\phi$, because the oracle's compressed covariance is worse-conditioned (E6).
The practical implication is that experimentalists need not measure the neuronal transfer function to apply the method.
Second, stimulation design matters but in a non-monotone way (E3, E7): too little leaves $\Sigma_{x,x}$ ill-conditioned and recovery fails; too much overrides the intrinsic dynamics that make the circuit biologically interesting; moderate stimulation of a modest neuron subset is the sweet spot.
Third, the dominant remaining error source is not the linear approximation but the network's own state-dependent intrinsic drive, modelled here as a CPG (E2). Closing this gap is the most promising direction for further accuracy.

The natural extensions are biological.
Applying the method to real recordings---\textit{C.~elegans} calcium imaging with NeuroPAL identification, larval zebrafish whole-brain imaging, lobster STG voltage imaging---would test whether synthetic-network performance transfers to circuits with heterogeneous cell types and mixed-sign connectivity.
Structured stimulation protocols, jointly estimated CPG models, and integration with anatomical priors from electron-microscopy connectomes are all natural follow-ons that the framework readily accommodates.
...
A hypothetical ``split'' oracle that estimated $\hat D$ separately and inverted $\hat\Sigma_{x,x}$ would coincide with the natural plug-in oracle in the population limit but inherit the better finite-sample conditioning of $\Sigma_{x,x}^{-1}$; Figure~\ref{fig:oracle} should be read as a comparison against the natural plug-in oracle, not the optimal one.
...
\begin{enumerate}
    ...
    \item \textbf{Off-path sign-cancellation.} The contraposition implicitly assumes off-path covariance contributions are non-negative; under anti-correlated CPG components or transient $\Sigma_{kj}<0$, the rule can fail in either direction (false zeros and false retentions). It is a conservative screening heuristic, not an identification theorem.
