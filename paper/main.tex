...
The primary comparison is against the \emph{Spectral} baseline: improvement from $0.120$ to $0.083$ ($31\%$), which isolates the contribution of the data-driven estimation beyond what the generative prior alone provides.
...
Crucially, the estimator achieves a $31\%$ improvement over an independent draw from the same generative prior ($0.083$ vs.\ $0.120$), demonstrating that the covariance accumulation framework extracts genuine connectivity information beyond what structural knowledge alone provides.
...
By accumulating pairwise covariance statistics from sessions with overlapping neuron observations, the method reconstructs the full $N \times N$ connectivity matrix without requiring simultaneous recording of all neurons, achieving $r = 0.90$ correlation between true and estimated weights and a $31\%$ error reduction over an independent draw from the same generative prior (matching sparsity pattern and spectral radius), demonstrating that the data-driven estimation extracts information beyond what structural knowledge alone provides.
At larger network sizes ($N = 300$, $T = 1000$), recovery error drops to $0.014$ ($97.5\%$ below the dense random baseline).
