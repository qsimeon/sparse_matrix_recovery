...
A nagging question in systems neuroscience is the relationship between \emph{structure} (the wiring of a neural circuit) and \emph{function} (the dynamics it produces) \citep{power2011functional}.
A long line of work attempts to infer one from the other---typically by fitting a simple recurrent neural network (RNN) model to recorded activity and reading off connectivity, or by simulating an anatomically reconstructed circuit and asking which dynamics it admits.
We pursue the inferential direction here, and (like that prior work) we adopt a simple RNN as our model class; we treat this restriction as a deliberate scope choice rather than an unmodeled assumption, and discuss its implications in Section~\ref{sec:discussion}.

The empirical reality this paper is built around is that even cutting-edge functional imaging never sees the whole circuit at once.
Limited optical field of view, sparse indicator expression, restricted optical access, and per-session photobleaching mean that each recording captures only a small, randomly chosen window of the underlying network \citep{ahrens2013whole, kato2015global, yemini2021neuropal}.
What an experimenter accumulates over time is therefore not a single complete recording of $N$ neurons, but a sequence of $K$ partial recordings, each with a different observed subset.
This holds across the small-circuit model systems where structure-function questions are most cleanly posed---the lobster stomatogastric ganglion (${\sim}30$ neurons; \citep{marder2007understanding}), \textit{C.~elegans} (302 neurons; \citep{white1986structure, cook2019whole}), and larval zebrafish \citep{ahrens2013whole}---where networks span tens to thousands of neurons and the partial-observation regime is the rule, not the exception.

We treat this as a matrix-recovery problem under repeated-trial sparse observation.
For a network of $N$ neurons with connectivity matrix $W \in \mathbb{R}^{N \times N}$, governed by discrete-time recurrent dynamics \citep{sussillo2009generating}, we ask: when each of $K$ sessions observes only a random subset of neurons, can the full $W$ be recovered by combining sessions?
Our setup makes a deliberately strong stationarity assumption---$W$ is fixed across sessions, no synaptic plasticity, no drift, identical noise statistics---which is biologically false in detail but a reasonable starting point for asking whether the partial-observation regime is in principle informative.
The key observation we exploit is that pairwise second-order statistics, unlike per-session models, accumulate across sessions: whenever neurons $i$ and $j$ are simultaneously observed in any session, that session contributes to estimating the $(i,j)$ entry of the empirical covariance, and a single global inversion at the end recovers $W$.

This setup also surfaces a \emph{control--estimation tradeoff} related to classical persistent excitation in system identification \citep{ljung1998system}: extrinsic stimulation aids identifiability by exciting network modes, but disrupts the intrinsic dynamics that make the circuit biologically meaningful.
Real circuits additionally contain intrinsic oscillators (central pattern generators; CPGs \citep{marder2001central}) whose detailed structure is unknown and acts as unmodeled autonomous drive.
Both factors enter the analysis as nuisance terms in the recovery error decomposition.
...
\begin{equation}
    ...
    \label{eq:error}
\end{equation}
For $\phi = \tanh$ and Gaussian states, the Stein--Price identity \citep{price1958useful, stein1981estimation} simplifies $E_1$ to $W(D - I)$ where $D$ is a diagonal matrix of expected derivative gains $D_{ii} = \mathbb{E}[\mathrm{sech}^2(x_i)] \in (0,1)$ (full derivation in Appendix~\ref{sec:app_stein}).
The mismatch is therefore proportional to how much $\tanh$ compresses each neuron's variance away from the linear regime, and shrinks to zero when state variance is small.
Both $E_1$ and $E_2$ are amplified by $\|\Sigma_{x,x}^{-1}\|_2$: ill-conditioned covariance (from sparse measurement) degrades recovery even with perfect model knowledge, and a key consequence---worked out in Appendix~\ref{sec:app_oracle}---is that an oracle estimator that uses the true $\phi$ ends up inverting an even \emph{more} ill-conditioned matrix than the linear approximation, an implicit-regularization gap we exploit in E6.
...
\textbf{(A--B)} PCA phase portraits of full-network activity ($X \in \mathbb{R}^{T \times N}$ projected onto PC1--PC2, colored by time via plasma colormap: dark purple $\to$ yellow). (A) Under autonomous dynamics ($\sigma=0$), the trajectory is confined to a low-dimensional manifold shaped by CPG oscillations. (B) Under stimulation ($\sigma=1.0$), the trajectory fills a larger volume in PC space, reflecting the expanded covariance coverage that enables estimation.
\textbf{(C)} Power spectral density (Welch's method) averaged across neurons. Autonomous dynamics (blue) show concentrated spectral peaks at CPG frequencies; stimulated dynamics (red) exhibit a flatter spectrum reflecting white-noise input.
\textbf{(D--E)} PCA phase portraits restricted to CPG nodes only, showing that CPG-driven subspace structure persists under both autonomous (D) and stimulated (E) conditions.
\textbf{(F)} Per-node activity variance under autonomous dynamics. CPG nodes (orange-red) have systematically higher variance than non-CPG nodes (teal), providing a detection signal.
\textbf{(G)} PCA phase portrait restricted to non-CPG nodes under autonomous dynamics, showing lower-amplitude fluctuations driven solely by recurrent input from CPG nodes.
\textbf{(H)} Error decomposition: $\|E_1\|_F/N$ (model mismatch) vs.\ $\|E_2\|_F/N$ (CPG--state correlation) vs.\ total estimation error $\|\hat{W} - W\|_F/N$. $E_1$ and $E_2$ are Frobenius norms of the individual error components (Eq.~\ref{eq:error}); the total can be smaller because these components partially cancel across matrix entries. For this representative topology, $E_2$ is ${\sim}2.8\times$ larger than $E_1$ (median across topologies: ${\sim}3\times$), confirming that CPG correlation is the dominant error source.
...
The approximate estimator (using $\Sigma_{x,x}$) outperforms the natural plug-in oracle that inverts $\Sigma_{\phi(x),x}$ with known $\phi$ and $b$.
The intuition is that the oracle's covariance is a \emph{row-compressed} version of $\Sigma_{x,x}$ by the diagonal gain matrix $D$ from Stein--Price, which is poorly conditioned whenever per-neuron state variances are heterogeneous; the linear approximation pays a small bias to avoid this conditioning penalty, in the same spirit as ridge regression versus OLS or James--Stein shrinkage versus the maximum-likelihood mean.
The full bias-variance breakdown---including a discussion of how a ``split'' oracle that separately estimated $D$ would close part of the gap---is in Appendix~\ref{sec:app_oracle}.
E6 (Figure~\ref{fig:oracle}) confirms the prediction empirically: the approximation matches or outperforms the oracle at every stimulation intensity tested, with the largest non-pathological gap at low stimulation.
...
We started from the observation that even cutting-edge functional imaging never sees a whole neural circuit at once: each recording is a small, randomly chosen window into a larger network.
The structure-function inference problem in that regime is rarely framed as a matrix-recovery problem, partly because per-session models cannot recover entries of $W$ that are never co-observed within any single session.
The contribution of this paper is to show that pooling pairwise second-order statistics across sessions \emph{before} any inversion turns the same partial observations into a tractable inverse problem solvable by a single global pseudoinverse, that this design empirically dominates per-session VAR/GLM fits, and that the resulting estimator inherits an unexpected implicit regularization from the linearization itself.

Beyond the method itself, our systematic experiments reveal three findings with practical implications.
First, the ``incorrect'' linear approximation matches or outperforms the oracle estimator with known nonlinearity at \emph{all} tested regimes (E6), a concrete instance of the James--Stein phenomenon where implicit regularization reduces total risk: experimentalists need not characterize the precise neuronal transfer function, since the simpler model is provably better-conditioned.
Second, a fundamental \emph{control-estimation tradeoff} governs experimental design (E3, E7): stimulation is necessary for identifiability but excessive stimulation degrades the covariance structure, with an optimal operating regime of moderate gain ($\sigma \approx 0.5$--$1.0$) applied to ${\sim}33\%$ of neurons---a practical target for optogenetic protocols.
Third, error decomposition (E2) reveals that CPG--state correlation (${\sim}3\times$ larger than model mismatch) is the dominant bottleneck, identifying autonomous-dynamics modeling, not nonlinearity refinement, as the route to further gains.

Two limitations frame what comes next.
First, like much structure-function inference work, we assume a simple RNN as the generative model for the underlying circuit; biological circuits exhibit conduction delays, mixed-sign synapses, cell-type heterogeneity, and short-timescale plasticity that this model class abstracts away.
Second, our entire analysis assumes \emph{stationarity}: the same $W$, the same noise statistics, and the same neuronal parameters across all $K$ sessions, with no synaptic plasticity or representational drift between recordings.
This assumption is biologically false in detail---weights change over hours to days, and even within a session---and is the price paid to make the multi-session pooling tractable.
It is, however, a reasonable starting point for asking whether the partial-observation regime is in principle informative; relaxing it (e.g., treating $W$ as slowly drifting and pooling within short coherent windows, or modeling per-session $W_k$ with a shared low-rank component) is the natural next step.

Several other directions merit investigation.
...
For small state variance, $1 - \mathbb{E}[\text{sech}^2(X_i)] \approx \sigma_i^2 - 2\sigma_i^4 + O(\sigma_i^6)$, giving an $O(\sigma^2)$ bound on $\|E_1\|_F$.
