...
\thanks{Code available at \url{https://github.com/qsimeon/sparse_matrix_recovery}}}
...
\emph{VAR}: a per-session vector autoregressive baseline (described below);
...
\paragraph{VAR baseline.}
A natural comparison is to a standard single-lag vector autoregressive (VAR) model.
For each session $k$ with $m$ observed neurons, we fit a VAR(1) on the observed submatrix:
$\hat{A}_k = \hat{\Sigma}^{(k)}_{y_{t+1},y_t}[\mathcal{M}_k] \cdot \left(\hat{\Sigma}^{(k)}_{y_t,y_t}[\mathcal{M}_k]\right)^{-1}$, then average element-wise over sessions where each pair is co-observed.
This ``invert-then-accumulate'' strategy contrasts with our ``accumulate-then-invert'' approach (Eq.~\eqref{eq:estimator}).
The key difference: our method pools the full $N\times N$ covariance structure before the single global inversion, leveraging indirect paths through the joint covariance matrix; the VAR inverts a separate $m\times m$ matrix per session, which discards the global covariance structure.
As shown in Figure~\ref{fig:granger}H, the VAR achieves error $0.136$, \emph{worse} than the Spectral prior ($0.120$), and $37\%$ worse than our accumulate-then-invert estimator ($0.086$), empirically confirming that pooling before inversion is the critical design choice.
...
E2 & Method ablation & Chance $\to$ VAR $\to$ Est $\to$ Granger & What does each step add? \\
...
\caption{\textbf{Effect of Granger-causality refinement and error structure} ($N=15$, $66\%$ measured, $\sigma=1.0$). \textbf{(A--D)} Weight matrix heatmaps from a representative topology: (A)~true $W$, (B)~raw covariance estimate $\hat{W}$, (C)~Granger-refined $\hat{W}$, (D)~absolute error $|W-\hat{W}|$. \textbf{(E)} Error-weight scatter: each off-diagonal entry's absolute error $|\hat{W}_{ij}-W_{ij}|$ vs.\ true weight $W_{ij}$.
...
\textbf{(H)} Recovery error with increasing prior knowledge: Chance $\to$ Adj $\to$ Spec $\to$ VAR $\to$ Est $\to$ Grn. The VAR baseline ($0.136$, per-session invert-then-accumulate) is $37\%$ worse than our accumulate-then-invert estimate ($0.086$), confirming that pooling before global inversion is the key design choice. $31\%$ improvement from Spec ($0.120$) to Granger ($0.083$).
\textbf{(I--K)} Violin plots over 10 topologies: Frobenius error (I), edge precision (J, $0.30\to0.40$), edge recall (K, maintained at $0.97$).}
\label{fig:granger}
\end{figure}
...
(3)~We compare against a per-session VAR baseline (E2), which is $37\%$ worse than our accumulate-then-invert estimator, confirming the key design choice. Comparison against other established methods such as Ridge-GLMs \citep{pillow2008spatio} or penalized VAR models would further strengthen the empirical evaluation.
...
All code, experiment configurations, and data are available at \url{https://github.com/qsimeon/sparse_matrix_recovery}.
