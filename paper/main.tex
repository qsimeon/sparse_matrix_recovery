...
With $n_s$ stimulated neurons receiving independent i.i.d.\ noise, the stimulation covariance has rank exactly $n_s$.
When $n_s = 0$, the rank is zero: all input directions in state space depend entirely on the CPG, leaving $\Sigma_{x,x}$ ill-conditioned and the estimator unreliable.
...
We compare three conditions: no stimulation ($0\%$), moderate coverage ($33\%$, $n_s = 10$ of $N=30$), and broader coverage ($66\%$, $n_s = 20$), at fixed $\sigma=1.0$ and $66\%$ measurement.
...
At $66\%$ coverage, performance is unchanged from $33\%$, confirming that the benefit saturates quickly (Figure~\ref{fig:sensors}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.7\linewidth]{figures/fig9_stim_fraction.pdf}
\caption{\textbf{Effect of stimulation coverage} on connectivity recovery ($N=30$, $T=1000$, $66\%$ measured, $\sigma=1.0$). Three conditions: no stimulation ($0\%$, pure CPG dynamics), moderate coverage ($33\%$, $n_s{=}10$), and broader coverage ($66\%$, $n_s{=}20$). Zero stimulation yields substantially degraded recovery (error ${\approx}0.83$) because $\Sigma_{x,x}$ has zero rank in the stimulation directions; $33\%$ coverage drops error to ${\approx}0.05$, matching baseline performance; $66\%$ shows no further improvement, confirming rapid saturation. Shaded: 95\% bootstrap CI. Dots: individual topologies ($n{=}10$). Stimulation fraction controls the \emph{rank} of the input covariance, independently of stimulation intensity (E3).}
\label{fig:sensors}
\end{figure}
...
This state dependence is the source of the $E_2$ error term in the estimation error decomposition (Section~\ref{sec:discussion}), which dominates the model mismatch $E_1$ by a factor of $\sim 2\times$.
...
\begin{tikzpicture}[
    ...
    box/.style={
        draw, rounded corners=4pt, minimum width=2.6cm, minimum height=1.1cm,
        align=center, font=\small, inner sep=6pt
    },
    statebox/.style={box, fill=blue!8, draw=blue!40!black},
    cpgbox/.style={box, fill=orange!10, draw=orange!60!black,
                   minimum width=3.4cm, minimum height=1.4cm},
    dynbox/.style={box, fill=red!8, draw=red!40!black, minimum width=3.8cm},
    sumnode/.style={circle, draw, minimum size=0.7cm, font=\large, inner sep=0pt},
    >=stealth, thick
]

%% Nodes
\node[statebox] (state) {$x_t$\\[2pt]{\footnotesize network state}};

\node[cpgbox, right=of state] (cpg)
    {\textbf{CPG}\\[3pt]
     {\footnotesize frozen $\cdot$ constructed once per topology}\\
     {\footnotesize shared across $K$ sessions}};

\node[sumnode, right=2.4cm of cpg] (plus) {$+$};

\node[statebox, above=1.3cm of plus] (bstim)
    {$b_t^{\mathrm{stim}}$\\[2pt]{\footnotesize i.i.d.\ noise}};

\node[dynbox, right=1.6cm of plus] (dyn)
    {$x_{t+1} = W\phi(x_t)+b_t$\\[3pt]{\footnotesize network dynamics}};

%% Forward arrows
\draw[->] (state) -- (cpg);
\draw[->] (cpg)   -- node[above, font=\small]{$b_t^{\mathrm{CPG}}$} (plus);
\draw[->] (bstim) -- (plus);
\draw[->] (plus)  -- (dyn);

%% State-dependence feedback (the E2 error source)
\draw[->, dashed, draw=red!70!black, line width=0.8pt]
    (dyn.south) -- ++(0,-0.9)
    -- node[below, font=\scriptsize, text=red!70!black]
       {$\mathrm{Cov}(b^{\mathrm{CPG}}_t,\, x_t)\neq 0\;\Rightarrow\;E_2\text{ error}$}
    (cpg.south |- dyn.south)
    -- (cpg.south);

\end{tikzpicture}
\caption{\textbf{Central Pattern Generator (CPG) role.}
The CPG maps the current network state $x_t$ to an intrinsic drive $b^{\mathrm{CPG}}_t$, which is summed with extrinsic stimulation $b^{\mathrm{stim}}_t$ to form the total network input $b_t$.
The CPG is frozen (non-trainable), constructed once per network topology, and shared across all $K$ recording sessions (each with an independent time counter).
Because its output depends on $x_t$, the CPG violates the independence assumption $\mathrm{Cov}(b_t,x_t)\approx 0$, producing the $E_2$ error term that dominates model mismatch $E_1$ by ${\sim}2\times$.}
