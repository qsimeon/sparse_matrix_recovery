...
First, autapses are not reported as distinct structural features in published connectomics reconstructions, including \textit{C.~elegans} \citep{white1986structure, cook2019whole} and \textit{Drosophila} circuit maps.
...
Note that $\|\Sigma_{x,x}^{-1}\|_2$ amplifies both error sources: ill-conditioned covariance (from sparse measurement) degrades recovery even with perfect model knowledge.
...
\textbf{Granger non-causality}: $W_{ij} = 0$ where $\Sigma_{x_t, x_t}(i,j) > \Sigma_{x_{t+1}, x_t}(i,j)$, indicating that the lagged covariance does not exceed the contemporaneous covariance. This sparsity pattern is the unique contribution of the Granger step.
...
The number of neurons receiving stimulation is a distinct experimental knob from how intensely they are stimulated; stimulation fraction controls the \emph{rank} of the stimulation covariance, while gain controls its \emph{magnitude}.
...
With zero stimulation ($0\%$), the network relies entirely on CPG-driven dynamics, yielding substantially degraded recovery, as the covariance matrix $\Sigma_{x,x}$ is ill-conditioned without independent noise input.
...
\textbf{(E)} PCA phase portrait under stimulation: the trajectory fills a larger volume in PC space, indicating that stochastic input excites network modes beyond those driven by CPGs, and this expanded coverage is what enables covariance-based estimation.
...
\textbf{(I)} Scatter plot of true off-diagonal weights $W_{ij}$ (horizontal) vs.\ Granger-refined estimates $\hat{W}_{ij}$ (vertical) for this representative topology. Pearson $r = 0.94$; dashed line: $y=x$. Median across all 10 topologies: $r = 0.90$.}
\label{fig:dynamics}
\end{figure}
...
Figure~\ref{fig:cpg_architecture} summarizes the CPG's role in the simulation.

\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{figures/fig11_cpg_architecture.pdf}
\caption{\textbf{Central Pattern Generator (CPG) architecture and role.}
The CPG maps the current network state $x_t$ to an intrinsic drive signal $b^{\mathrm{CPG}}_t$, which is added to the extrinsic stimulation $b^{\mathrm{stim}}_t$ to form the total input to the network.
The CPG is a frozen (non-trainable) function combining a circularly-shifted state input, a random deep network, and a chaotic reservoir drive.
It is constructed once per network topology and shared (with independent time counters) across all $K$ recording sessions.
The state-dependence $\mathrm{Cov}(b^{\mathrm{CPG}}_t, x_t) \neq 0$ is the source of the $E_2$ estimation error, which dominates model mismatch $E_1$ by ${\sim}2\times$ (median across topologies).}
\label{fig:cpg_architecture}
\end{figure}
