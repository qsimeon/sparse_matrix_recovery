...
A Granger-causality refinement step enforces biological constraints via projected gradient descent, achieving $r = 0.96$ correlation with true connectivity and $84\%$ improvement over chance using only $66\%$ neuron coverage per session.
...
Error decomposition reveals that correlation from central pattern generators, not model mismatch, is the dominant error bottleneck ($3.1\times$ larger), identifying intrinsic dynamics modeling as the key target for further improvement.
\end{abstract}
...
First, autapses are rarely identified in connectomics reconstructions---most electron microscopy (EM) studies across organisms (\textit{C.~elegans}, \textit{Drosophila}, mouse cortex) do not report self-synapses as a distinct structural feature.
...
This is motivated by the measurement modality: EM connectomics---the primary source of ground-truth wiring data---measures synapse \emph{size} (contact area, vesicle count) but not synapse \emph{sign} (excitatory vs.\ inhibitory), yielding an inherently unsigned weight matrix.
...
We run 20 repetitions per parameter setting (20 distinct random topologies) to measure variability across circuits.
The two levels serve distinct purposes: the 50 sessions per topology are necessary for covariance accumulation; the 20 topologies give error bars over the space of possible circuits.
...
\midrule
E1 & Network size $N$, duration $T$ & $N \in \{8,15,30\}$, $T \in \{100,500,1000\}$ & How does recovery scale? \\
...
E7 & Stimulation fraction & $0$--$100\%$ & How many neurons to stimulate? \\
\bottomrule
\end{tabular}
\end{table}
...
The covariance estimator (with diagonal zeroed) consistently outperforms the chance baseline, achieving median errors of $0.05$--$0.19$ vs.\ $\sim 0.54$ for chance.
...
For $N=30$ with $T=1000$, the method achieves excellent recovery: $0.050$ (Granger-refined), a $91\%$ improvement over chance (Figure~\ref{fig:scaling}).
...
\midrule
8 & 100 & 0.535 & 0.155 & \textbf{0.148} & 5\% \\
8 & 500 & 0.535 & 0.133 & \textbf{0.120} & 10\% \\
8 & 1000 & 0.535 & 0.128 & \textbf{0.120} & 6\% \\
15 & 100 & 0.543 & 0.140 & \textbf{0.110} & 21\% \\
15 & 500 & 0.543 & 0.094 & \textbf{0.084} & 11\% \\
15 & 1000 & 0.543 & 0.094 & \textbf{0.083} & 11\% \\
30 & 100 & 0.562 & 0.226 & \textbf{0.130} & 42\% \\
30 & 500 & 0.562 & 0.063 & \textbf{0.054} & 13\% \\
30 & 1000 & 0.562 & 0.060 & \textbf{0.053} & 12\% \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{Scaling properties of the covariance estimator.} \textbf{(Left)} Median recovery error (Frobenius distance / $N$) vs.\ recording duration $T$ for three network sizes ($N \in \{8, 15, 30\}$) with $66\%$ measurement. Shaded regions show 95\% bootstrap confidence intervals over 20 random topologies (50 sessions each). Error generally decreases with $T$ and $N$. The best result is at $N=30$, $T=1000$: Granger-refined error of $0.053$. \textbf{(Right)} Error vs.\ network size $N$ at fixed recording durations $T \in \{100, 500, 1000\}$.}
\label{fig:scaling}
\end{figure}

\subsection{Granger Refinement (E2)}

We evaluate the Granger-causality refinement step on $N=15$ networks with $T=1000$, $66\%$ measurement, and 20 network topologies.
After enforcing the no-autapse prior ($W_{ii} = 0$, a known structural constraint applied to the raw estimate), the Granger refinement further reduces error by an additional $\sim 8\%$.
...
Crucially, the estimator achieves an $84\%$ improvement over the chance baseline ($0.084$ vs.\ $0.543$), demonstrating that the covariance accumulation framework extracts genuine connectivity information.
...
\caption{\textbf{Effect of Granger-causality refinement} ($N=15$, $66\%$ measured, $\sigma=1.0$, 50 instances). \textbf{(A--D)} Weight matrix heatmaps from a representative topology: (A)~true $W$, (B)~raw covariance estimate $\hat{W}$, (C)~Granger-refined $\hat{W}$, (D)~absolute error. The covariance estimate captures the overall structure; Granger refinement sharpens it by enforcing sparsity and non-negativity. \textbf{(E--G)} Quantitative evaluation over 50 random topologies: (E)~Frobenius error (estimate already has diagonal zeroed) reduced from $0.089$ to $0.084$ with Granger, (F)~edge detection precision, (G)~recall achieves perfect median $1.0$. Violin plots with individual topologies overlaid. \textbf{(H)} Recovery error with increasing prior knowledge of the ground truth: Chance (no knowledge), Adj (know sparsity pattern), Spec (know sparsity + spectral radius), Est (covariance-based, no oracle knowledge), Grn (Granger-refined with biological constraints). From chance ($0.543$) to Granger-refined ($0.084$): $84\%$ total improvement.}
\label{fig:granger}
\end{figure}
...
Full observability ($100\%$) yields error of $0.084$ compared to $0.321$ at $33\%$ measurement (Granger-refined), confirming that measurement sparsity is a significant challenge.
At $33\%$ measurement, the raw estimate ($0.590$) is substantially worse due to ill-conditioning; the Granger refinement reduces this to $0.321$ ($46\%$ improvement).
...
Surprisingly, $\tanh$ yields the \emph{best} recovery (Granger error $0.085$), while identity and ReLU---for which the linear approximation is exact---perform worse (error $\sim 0.17$).
...
Sigmoid achieves intermediate error ($0.133$): its boundedness helps conditioning relative to identity and ReLU, but its non-zero mean ($\phi(0) = 0.5$) biases the covariance estimate.
...
\caption{\textbf{Robustness to nonlinearity mismatch} for $N=15$, $T=1000$, $66\%$ measured. \textbf{(Left)} The four activation functions tested. \textbf{(Right)} Median recovery error (log scale) for the raw estimator (blue) and Granger-refined (green) with bootstrap error bars. Counterintuitively, $\tanh$ yields the \emph{best} recovery ($0.085$) because it bounds state values, improving $\kappa(\Sigma_{x,x})$. Unbounded nonlinearities (identity, ReLU) allow state drift that worsens conditioning. Dots show individual topologies ($n=20$).}
...
We test this by comparing both estimators across stimulation intensities $\sigma \in \{0, 0.1, 0.25, 0.5, 1, 2, 5\}$ for $N=15$, $T=1000$ with $\tanh$ nonlinearity, $66\%$ measurement, and 20 topologies.

The approximation outperforms or matches the oracle at every tested stimulation level (Figure~\ref{fig:oracle}).
The oracle penalty is largest at moderate stimulation ($3.6\times$ at $\sigma = 0.5$) and narrows to $1.4$--$1.7\times$ for $\sigma \geq 1$.
...
\caption{\textbf{Oracle vs.\ approximation comparison} across stimulation intensities ($N=15$, $T=1000$, $66\%$ measured, $\tanh$, 20 topologies $\times$ 50 instances). \textbf{(A)} Median recovery error (log scale) for the oracle estimator using the true nonlinearity (red), the linear approximation (blue), and the Granger-refined approximation (green). Shaded: 95\% bootstrap CI. Dots: individual topologies. The approximation matches or outperforms the oracle at every $\sigma$, with the largest gap at $\sigma=0.5$ ($3.6\times$). \textbf{(B)} Oracle penalty factor (ratio of oracle to approximation error). The oracle never achieves lower error than the approximation, confirming the implicit regularization effect predicted by the bias-variance analysis (Appendix~\ref{sec:app_oracle}).}
\label{fig:oracle}
\end{figure}
...
We vary the number of stimulated neurons at fractions $\{0\%, 33\%, 47\%, 67\%, 100\%\}$ of $N=15$ (i.e., $n_s \in \{0, 5, 7, 10, 15\}$) at fixed $\sigma=1.0$ and $66\%$ measurement.
With zero stimulation ($0\%$), the network relies entirely on CPG-driven dynamics, yielding poor recovery (Granger-refined error $0.314$)---the covariance matrix $\Sigma_{x,x}$ is ill-conditioned without independent noise input.
At $33\%$ stimulation ($5$ neurons), error drops dramatically to $0.085$, matching the baseline performance.
Beyond $33\%$, performance plateaus near $0.084$, indicating that moderate stimulation coverage suffices when stimulation gain is adequate (Figure~\ref{fig:sensors}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.7\linewidth]{figures/fig9_stim_fraction.pdf}
\caption{\textbf{Effect of stimulation coverage} on connectivity recovery for $N=15$, $T=1000$, $66\%$ measured, $\sigma=1.0$. Median recovery error vs.\ fraction of neurons receiving extrinsic stimulation (stimulation fraction). Stimulating just $1$ neuron ($8\%$) yields high error with large variance; $2$ sensors ($17\%$) halves the error; beyond $33\%$, performance plateaus. Shaded: 95\% bootstrap CI. Dots: individual topologies ($n=20$). Sensor coverage controls the \emph{rank} of the stimulation covariance, distinct from stimulation intensity (E3).}
\label{fig:sensors}
\end{figure}
...
The E7 results show that zero stimulation yields poor recovery (error $0.314$) because CPG-driven dynamics alone leave $\Sigma_{x,x}$ ill-conditioned.
At $33\%$ stimulation ($5$ of $N=15$ neurons), error drops to $0.085$, and performance plateaus beyond $\sim 33\%$ stimulation coverage.
...
The gap is largest at moderate stimulation ($3.6\times$ at $\sigma=0.5$) and narrows at high stimulation ($1.4$--$1.7\times$ for $\sigma \geq 1$) as both estimators degrade.
...
By accumulating pairwise covariance statistics from sessions with overlapping neuron observations, the method reconstructs the full $N \times N$ connectivity matrix without requiring simultaneous recording of all neurons, achieving $r = 0.96$ correlation between true and estimated weights and an $84\%$ improvement over chance at our baseline configuration.
