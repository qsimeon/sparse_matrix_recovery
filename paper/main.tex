...
\emph{VAR}: per-session vector autoregressive baseline (invert-then-accumulate);
\emph{GLM}: per-neuron ridge-regularized linear model baseline ($\alpha=1$);
...
\paragraph{VAR and GLM baselines.}
Two complementary baselines test alternative estimation strategies.
The \emph{VAR} baseline fits a single-lag VAR(1) on the observed sub-network per session:
$\hat{A}_k = \hat{\Sigma}^{(k)}_{y_{t+1},y_t}[\mathcal{M}_k] \cdot \left(\hat{\Sigma}^{(k)}_{y_t,y_t}[\mathcal{M}_k]\right)^{-1}$, then averages element-wise over co-observed sessions.
This ``invert-then-accumulate'' strategy discards the joint $N\times N$ covariance structure that our global inversion exploits.
The \emph{GLM} baseline fits a per-neuron ridge regression ($\alpha{=}1$):
for each neuron $i$ and session $k$ where $i$ is measured,
$w_i^{(k)} = \bigl(\hat{\Sigma}^{(k)}_{y_t,y_t}[\mathcal{M}_k] + \tfrac{\alpha}{T}\mathbf{I}\bigr)^{-1} \hat{\Sigma}^{(k)}_{y_{t+1},y_t}[\mathcal{M}_k,i]$, then averages element-wise.
Note that with $\alpha{=}0$, the GLM reduces to the VAR row-by-row (both are per-session OLS); the ridge penalty introduces a bias toward zero, analogous to implicit regularization in our estimator but applied locally per session.
As shown in Figure~\ref{fig:granger}H, the VAR achieves error $0.136$ (worse than even the Spectral prior), and the Ridge-GLM achieves $0.117$ (slightly better than Spectral due to the regularization bias, but $26\%$ worse than our estimator at $0.086$). Both confirm that pooling before the global inversion is the critical design choice.
...
% ============================================================================
\paragraph{Code and preprint availability.}
All code, experiment configurations, and data are available at \url{https://github.com/qsimeon/sparse_matrix_recovery}.
A preprint of this work is available at \url{https://arxiv.org/abs/2603.18497}.
