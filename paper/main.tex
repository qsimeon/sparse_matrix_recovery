...
\begin{abstract}
In many neural circuits---from \textit{C.~elegans} to cortical organoids---no single recording session captures the full network.
How, then, can partial snapshots be stitched into a complete wiring diagram?
We present a covariance-based framework that accumulates pairwise statistics across sessions with overlapping neuron subsets, recovering the connectivity matrix of a recurrent neural network without requiring full observability.
A Granger-causality refinement step enforces biological sparsity constraints via projected gradient descent.
Through systematic experiments on synthetic networks, we uncover three findings with practical implications:
(i)~a fundamental \emph{control-estimation tradeoff} in which stimulation aids identifiability but disrupts intrinsic dynamics;
(ii)~an implicit regularization effect in which the deliberately ``incorrect'' linear approximation outperforms the oracle estimator with known nonlinearity at all tested regimes---a concrete instance of James--Stein shrinkage;
and (iii)~an error decomposition revealing that correlation from intrinsic oscillatory dynamics, not model mismatch, is the dominant estimation bottleneck, identifying autonomous dynamics modeling as the primary target for further improvement.
\end{abstract}
