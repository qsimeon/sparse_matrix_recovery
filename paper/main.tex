...
A Granger-causality refinement step enforces biological constraints via projected gradient descent, achieving strong correlation with true connectivity and substantial improvement over chance baselines using partial neuron coverage per session.
Through systematic experiments on synthetic networks modeling neural circuits, we characterize a fundamental \emph{control-estimation tradeoff}: stimulation aids identifiability but disrupts intrinsic dynamics, with the optimal level depending on measurement density.
...
Error decomposition reveals that correlation from central pattern generators, not model mismatch, is the dominant error bottleneck, identifying intrinsic dynamics modeling as the primary target for further improvement.
\end{abstract}
...
For $N \leq 30$, recovery generally improves with $T$ (saturating for $N{=}15$ around $T{=}500$), with the best small-network result at $N{=}30$, $T{=}1000$ (error $0.052$, a $91\%$ improvement over chance).
At $N{=}159$, recovery reaches $0.035$ at $T{=}1000$ ($94\%$ improvement over chance), the best across all tested sizes.
...
\begin{table}[ht]
\centering
\caption{Median Granger-refined recovery error (Frobenius / $N$) across network sizes and durations ($66\%$ measurement, 10 topologies, 50 sessions each). Bold indicates the best result per $N$. Chance baseline ranges from $0.544$ ($N{=}15$) to $0.577$ ($N{=}1074$). For $N \leq 30$, error generally decreases with $T$; larger networks exhibit higher variance due to covariance ill-conditioning (see text). Best overall: $N{=}159$, $T{=}1000$ ($0.035$, $94\%$ improvement over chance).}
\label{tab:baseline}
\begin{tabular}{cccccc}
\toprule
$N$ & $T{=}100$ & $T{=}250$ & $T{=}500$ & $T{=}750$ & $T{=}1000$ \\
\midrule
15   & 0.107 & 0.091 & 0.083 & 0.082 & \textbf{0.080} \\
30   & 0.091 & 0.059 & 0.054 & 0.053 & \textbf{0.052} \\
159  & 0.104 & 0.093 & 0.085 & 0.044 & \textbf{0.035} \\
300  & 0.132 & 0.175 & 0.129 & \textbf{0.086} & 0.149 \\
1074 & 0.384 & 0.349 & \textbf{0.257} & 0.265 & 0.296 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{Scaling properties of the covariance estimator.} \textbf{(Left)} Median recovery error (Frobenius distance / $N$) vs.\ recording duration $T$ for five network sizes ($N \in \{15, 30, 159, 300, 1074\}$) with $66\%$ measurement. Shaded regions show 95\% bootstrap confidence intervals over 10 random topologies (50 sessions each). For $N \leq 30$, error generally decreases with $T$ (saturating around $T{=}500$ for $N{=}15$); for $N \geq 300$, covariance ill-conditioning produces higher variance and non-monotonic behavior. Best results: $N{=}159$ ($0.035$) and $N{=}30$ ($0.052$) at $T{=}1000$. \textbf{(Right)} Error vs.\ network size $N$ at fixed recording durations $T \in \{100, 250, 500, 750, 1000\}$.}
\label{fig:scaling}
\end{figure}
...
Crucially, the estimator achieves an $85\%$ improvement over the chance baseline ($0.086$ vs.\ $0.555$), demonstrating that the covariance accumulation framework extracts genuine connectivity information.
...
At full measurement ($100\%$), a striking pattern emerges: \emph{zero stimulation} yields substantially degraded recovery, while moderate stimulation ($\sigma \approx 0.5$) yields the best performance.
...
Full observability ($100\%$) yields error of $0.083$ compared to $0.286$ at $33\%$ measurement (Granger-refined), confirming that measurement sparsity is a significant challenge.
At $33\%$ measurement, the raw estimate ($0.461$) is substantially worse due to ill-conditioning; the Granger refinement reduces this to $0.286$ ($38\%$ improvement).
...
Surprisingly, $\tanh$ yields the \emph{best} recovery (Granger error $0.083$), while identity and ReLU---for which the linear approximation is exact---perform worse (error $\sim 0.17$).
...
Sigmoid achieves intermediate error ($0.129$): its boundedness helps conditioning relative to identity and ReLU, but its non-zero mean ($\phi(0) = 0.5$) biases the covariance estimate.
...
The oracle penalty is largest at low-to-moderate stimulation ($\sim5$--$6\times$ at $\sigma = 0.25$--$0.5$) and narrows to $1.3$--$1.7\times$ for $\sigma \geq 1$.
...
The gap is largest at low-to-moderate stimulation ($\sim5$--$6\times$ at $\sigma = 0.25$--$0.5$) and narrows at high stimulation ($1.3$--$1.7\times$ for $\sigma \geq 1$) as both estimators degrade.
...
Quantitatively, $\|E_2\|_F$ (input correlation) is ${\sim}2\times$ larger than $\|E_1\|_F$ (model mismatch), making CPG correlation the \emph{dominant} error source, not the linear approximation (Figure~\ref{fig:dynamics}H).
...
\textbf{(H)} Error decomposition: $\|E_1\|_F/N$ (model mismatch, $0.073$) vs.\ $\|E_2\|_F/N$ (CPG--state correlation, $0.229$) vs.\ total estimation error $\|\hat{W} - W\|_F/N$ ($0.076$). $E_1$ and $E_2$ are Frobenius norms of the \emph{individual error components} (Eq.~\ref{eq:error}); the total can be smaller because these components partially cancel in different matrix entries.  $E_2$ is ${\sim}2\times$ larger than $E_1$, confirming that CPG correlation---not the linear approximation---is the dominant error source.
...
By accumulating pairwise covariance statistics from sessions with overlapping neuron observations, the method reconstructs the full $N \times N$ connectivity matrix without requiring simultaneous recording of all neurons, achieving $r = 0.90$ correlation between true and estimated weights and an $85\%$ improvement over chance at the baseline configuration, improving to over $97\%$ at larger network sizes ($N = 300$, $T = 1000$).
...
Third, error decomposition reveals that CPG--state correlation (${\sim}2\times$ larger than model mismatch) is the dominant bottleneck, suggesting that future gains will come from modeling intrinsic dynamics rather than refining the nonlinear approximation.
...
\begin{tabular}{lccc}
    \toprule
    $N$ & DOF ratio & $\kappa(\hat{\Sigma}_{x,x})$ & Quality \\
    \midrule
    15  & 222    & $\sim 30$  & Overdetermined \\
    159 & 1.98   & $\sim 500$ & Borderline \\
    300 & 0.56   & $\sim 10^3$--$10^4$ & Underdetermined \\
    \bottomrule
\end{tabular}
\end{center}

When the DOF ratio falls below 1, the sample covariance matrix is rank-deficient relative to the true dimensionality, and the pseudoinverse amplifies estimation noise.
This explains the non-monotonic error and high variance observed at large $N$ in E1 (Figure~\ref{fig:scaling}), and is a fundamental limitation of covariance-based methods in high dimensions~\citep{ledoit2004well}.
...
\begin{enumerate}
    ...
    \item \textbf{Random deep network}: A 3-layer network with ReLU activations, RMS normalization, and absolute-value nonlinearities, with randomly initialized (frozen) weights. Xavier-style initialization with $\sigma = 1/\sqrt{d}$ prevents vanishing/exploding activations.
...
This state dependence is the source of the $E_2$ error term in the estimation error decomposition (Section~\ref{sec:discussion_error}), which dominates the model mismatch $E_1$ by a factor of $\sim 3\times$.
...
Each session starts with an independent time counter, so the CPG produces different trajectories due to different network states, measurement masks, and stimulation patterns---but the underlying oscillatory function is the same.
