...
Connectomics has historically split along two paths.
Anatomical connectomes (electron microscopy, expansion microscopy) give a wiring diagram but no synaptic weights and no dynamics.
Functional connectomes (calcium imaging, voltage imaging) give activity but assume the imaging FOV captures the whole circuit, an assumption that fails in practice for the small invertebrate circuits where causal reconstruction would matter most.
A third path---reconstructing weighted connectivity directly from \emph{sub-sampled} functional recordings---has been explored only sporadically: most prominently \citet{soudry2015shotgun} introduced the ``shotgun'' multi-session sampling design and inferred connectivity via expectation--maximisation on a generalised linear model.
We argue this idea is more powerful than its limited adoption suggests, and that the central design lever has been overlooked: \emph{accumulate the second-order statistics first, then invert once globally.}
The mathematical content of this paper is unusually simple---one pseudoinverse, one projected gradient---but the resulting estimator turns the partial-observation problem from intractable into nearly trivial whenever a coverage condition (every neuron pair co-observed in at least one session) is met.
Two surprising findings emerge from this simplicity, neither of them about the algorithm itself: a linear approximation outperforms an oracle that knows the true nonlinearity, and the dominant remaining error is not model mismatch but unmodelled intrinsic-circuit dynamics.
