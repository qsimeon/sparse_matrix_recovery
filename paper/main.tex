...
Recovery error is evaluated against a hierarchy of reference estimates, each incorporating additional knowledge of the ground truth (see E2 ablation, Figure~\ref{fig:granger}H):
\emph{Chance}: a dense random non-negative matrix $W_{\mathrm{rand}} \sim U(0,1)^{N \times N}$ (no structural knowledge);
\emph{Adjacency}: random $U(0,1)$ weights placed on the true sparsity pattern (knows which edges exist, but not their weights);
\emph{Spectral}: an independent draw from the same generative distribution as $W$ (true sparsity pattern, random weights, scaled to the true spectral radius $\rho(W)$)---this is the fairest ``chance'' comparison, representing what an experimenter could achieve by guessing from the prior;
\emph{Estimate}: our covariance-based method (no oracle knowledge);
\emph{Granger}: estimate refined with Granger non-causality zeros.
When we report ``improvement over chance'' ($85\%$, using the dense random baseline), this represents the total information extracted by the method compared to having no knowledge of the circuit.
The more informative comparison is against the \emph{Spectral} baseline: improvement from $0.120$ to $0.083$ ($31\%$), which isolates the contribution of the data-driven estimation beyond what the generative prior alone provides.
...
After enforcing the no-autapse prior ($W_{ii} = 0$) and non-negativity ($W_{ij} \geq 0$) on the raw estimate, the Granger refinement further reduces error by an additional $\sim 4\%$ through its non-causality sparsity pattern.
...
The Granger refinement improves precision to $0.40$ while maintaining recall at $0.97$, eliminating spurious edges without sacrificing true connectivity.
Crucially, the estimator achieves an $85\%$ improvement over the chance baseline ($0.083$ vs.\ $0.555$), demonstrating that the covariance accumulation framework extracts genuine connectivity information.
...
At $33\%$ measurement, the raw estimate ($0.110$) is modestly improved by the Granger refinement to $0.105$.
...
\caption{\textbf{Robustness to nonlinearity mismatch} for $N=15$, $T=1000$, $66\%$ measured. \textbf{(Left)} The four activation functions tested. \textbf{(Right)} Median recovery error (log scale) for the raw estimator (blue) and Granger-refined (green) with bootstrap error bars. Counterintuitively, $\tanh$ yields the \emph{best} recovery ($0.083$) because it bounds state values, improving $\kappa(\Sigma_{x,x})$. Unbounded nonlinearities (identity, ReLU) allow state drift that worsens conditioning. Dots show individual topologies ($n=10$).}
...
The oracle penalty is largest at low stimulation ($\sim2.7\times$ at $\sigma = 0$, $\sim1.7\times$ at $\sigma = 0.25$--$0.5$) and narrows to $1.5$--$1.6\times$ for $\sigma \geq 1$.
...
The Granger refinement's unique contribution is the non-causality sparsity pattern ($W_{ij} = 0$ where contemporaneous covariance exceeds lagged), improving precision from $0.30$ to $0.40$ while maintaining high recall ($0.97$).
This indicates that the Granger non-causality criterion effectively removes spurious edges without discarding true connections.
...
The gap is largest at low stimulation ($\sim2.7\times$ at $\sigma = 0$, $\sim1.7\times$ at $\sigma = 0.25$--$0.5$) and narrows at high stimulation ($1.5$--$1.6\times$ for $\sigma \geq 1$) as both estimators degrade.
...
\textbf{(H)} Error decomposition: $\|E_1\|_F/N$ (model mismatch, $0.080$) vs.\ $\|E_2\|_F/N$ (CPG--state correlation, $0.225$) vs.\ total estimation error $\|\hat{W} - W\|_F/N$ ($0.087$). $E_1$ and $E_2$ are Frobenius norms of the \emph{individual error components} (Eq.~\ref{eq:error}); the total can be smaller because these components partially cancel in different matrix entries.  For this representative topology, $E_2$ is ${\sim}3\times$ larger than $E_1$ (median across topologies: ${\sim}2\times$), confirming that CPG correlation---not the linear approximation---is the dominant error source.
...
In systematic tests (E8, 10 topologies, baseline configuration), at $\sigma_\epsilon = 0.1$ (relative to state std $\sim 0.8$) the Granger-refined error increases by $\sim 1\%$, and at $\sigma_\epsilon = 0.5$ by $\sim 33\%$.
...
(5)~Our primary results assume noiseless observations; noise robustness tests (E8) show graceful degradation ($1\%$ at $\sigma_\epsilon = 0.1$, $33\%$ at $\sigma_\epsilon = 0.5$).
...
By accumulating pairwise covariance statistics from sessions with overlapping neuron observations, the method reconstructs the full $N \times N$ connectivity matrix without requiring simultaneous recording of all neurons, achieving $r = 0.90$ correlation between true and estimated weights.
Compared to a dense random baseline (no structural knowledge), the method achieves $85\%$ error reduction; compared to an independent draw from the same generative prior (matching sparsity and spectral radius), it achieves $31\%$ error reduction---demonstrating that the data-driven estimation extracts information beyond what the structural prior alone provides.
At larger network sizes ($N = 300$, $T = 1000$), improvement over the dense baseline exceeds $97\%$.
...
This state dependence is the source of the $E_2$ error term in the estimation error decomposition (Section~\ref{sec:discussion_error}), which dominates the model mismatch $E_1$ by a factor of $\sim 2\times$.
