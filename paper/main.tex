...
Table~\ref{tab:baseline} shows recovery error across network sizes $N \in \{8, 15, 30\}$ and recording durations $T \in \{100, 500, 1000\}$ with $66\%$ measurement.
...
Full observability ($100\%$) yields error of $0.083$ compared to $0.213$ at $33\%$ measurement (Granger-refined), confirming that measurement sparsity is a significant challenge.
At $33\%$ measurement, the raw estimate ($0.345$) is substantially worse due to ill-conditioning; the Granger refinement reduces this to $0.213$ ($38\%$ improvement).
...
Surprisingly, $\tanh$ yields the \emph{best} recovery (Granger error $0.084$), while identity and ReLU---for which the linear approximation is exact---perform worse (error $\sim 0.17$).
...
\caption{\textbf{Robustness to nonlinearity mismatch} for $N=15$, $T=1000$, $66\%$ measured. \textbf{(Left)} The four activation functions tested. \textbf{(Right)} Median recovery error (log scale) for the raw estimator (blue) and Granger-refined (green) with bootstrap error bars. Counterintuitively, $\tanh$ yields the \emph{best} recovery ($0.084$) because it bounds state values, improving $\kappa(\Sigma_{x,x})$. Unbounded nonlinearities (identity, ReLU) allow state drift that worsens conditioning. Dots show individual topologies ($n=20$).}
...
The oracle penalty is largest at moderate stimulation ($4.5\times$ at $\sigma = 0.5$) and narrows to $1.4$--$1.7\times$ for $\sigma \geq 1$.
...
With zero stimulation ($0\%$), the network relies entirely on CPG-driven dynamics, yielding poor recovery (Granger-refined error $\sim 0.50$, near chance)---the covariance matrix $\Sigma_{x,x}$ is ill-conditioned without independent noise input.
At $33\%$ stimulation ($5$ neurons), error drops dramatically to $0.084$, matching the baseline performance.
Beyond $33\%$, performance plateaus near $0.083$, indicating that moderate stimulation coverage suffices when stimulation gain is adequate (Figure~\ref{fig:sensors}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.7\linewidth]{figures/fig9_stim_fraction.pdf}
\caption{\textbf{Effect of stimulation coverage} on connectivity recovery for $N=15$, $T=1000$, $66\%$ measured, $\sigma=1.0$. Median recovery error vs.\ fraction of neurons receiving extrinsic stimulation. Zero stimulation ($0\%$, pure CPG dynamics) fails catastrophically (error $\sim 0.50$, near chance); $33\%$ stimulation drops error to $0.084$; beyond $33\%$, performance plateaus. Shaded: 95\% bootstrap CI. Dots: individual topologies ($n=20$). Stimulation coverage controls the \emph{rank} of the input covariance, distinct from stimulation intensity (E3).}
\label{fig:sensors}
\end{figure}
...
The E7 results show that zero stimulation yields poor recovery (error $\sim 0.50$, near chance) because CPG-driven dynamics alone leave $\Sigma_{x,x}$ ill-conditioned.
At $33\%$ stimulation ($5$ of $N=15$ neurons), error drops to $0.084$, and performance plateaus beyond $\sim 33\%$ stimulation coverage.
This has a direct practical implication: in optogenetic experiments on small circuits, an experimenter need not achieve pan-neuronal actuator expression---targeting $3$--$5$ neurons in a 15-neuron circuit suffices, provided stimulation intensity is moderate ($\sigma \approx 0.5$--$1.0$).
...
The gap is largest at moderate stimulation ($4.5\times$ at $\sigma=0.5$) and narrows at high stimulation ($1.4$--$1.7\times$ for $\sigma \geq 1$) as both estimators degrade.
