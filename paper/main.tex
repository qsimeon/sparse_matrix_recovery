...
\begin{tikzpicture}[
    ...
    box/.style={
        draw, rounded corners=4pt, minimum width=2.0cm, minimum height=0.9cm,
    ...
    dynbox/.style={box, fill=red!8, draw=red!40!black,
                   minimum width=3.0cm, minimum height=0.9cm},
    sumnode/.style={circle, draw, minimum size=0.6cm, font=\normalsize, inner sep=0pt},
    >=stealth, thick
]

%% Nodes
\node[statebox] (state) {$x_t$\\[1pt]{\small network state}};

\node[cpgbox, right=of state] (cpg)
    {\textbf{CPG}\\[2pt]
     {\small frozen $\cdot$ one per topology}\\
     {\small shared $K$ sessions}};

\node[sumnode, right=1.8cm of cpg] (plus) {$+$};

\node[statebox, above=1.0cm of plus] (bstim)
    {$b_t^{\mathrm{stim}}$\\[1pt]{\small i.i.d.\ noise}};

\node[dynbox, right=1.4cm of plus] (dyn)
    {$x_{t+1} = W\phi(x_t)+b_t$\\[2pt]{\small network dynamics}};
...
\draw[->, dashed, draw=red!70!black, line width=0.8pt]
    (dyn.south) -- ++(0,-0.75)
    -- node[below, font=\small, text=red!70!black]
