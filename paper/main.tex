...
This work is an instance of a general system-identification problem with a particularly stringent observability constraint: \emph{the experimenter never sees the whole system at once}.
At any given moment one observes only a small window---a subset of state variables---and the windows change between sessions.
Across a long enough sequence of sessions one can hope to have observed every variable many times, and every \emph{pair} of variables at least once together; the question we study is whether that pairwise co-observability is sufficient, in principle and in practice, to identify the full connectivity matrix of an unknown recurrent network.
This is meaningful only under a no-drift assumption---the underlying weights are taken to be constant across sessions---which is acknowledged as an idealization rather than a fact, but is the working assumption that the rest of this paper rests on.

The motivating instance is functional neuroimaging.
Even cutting-edge whole-brain techniques in \textit{C.~elegans}, larval zebrafish, and \textit{Drosophila} \citep{ahrens2013whole, white1986structure, cook2019whole, quadrato2017cell, marder2007understanding} routinely fail to record an entire circuit simultaneously due to limited field of view, sparse indicator expression, or restricted optical access; what an experimentalist actually has, at the end of a campaign, is a stack of overlapping but never-complete partial recordings.
This stack is exactly the structure our method consumes.
Beyond neuroscience, the same partial-window-with-no-drift setting arises in any sub-sampled multi-session probing of a recurrent dynamical system, but we frame the work and run our experiments in the neuroscience context where it most concretely appears.

Two factors complicate the inversion. First, a \emph{control-estimation tradeoff} related to classical persistent excitation \citep{ljung1998system}: stimulating neurons with random perturbations excites otherwise-unobservable network modes (good for identifiability) but disrupts the intrinsic dynamics that make the circuit biologically meaningful (bad for studying it). Second, real circuits carry intrinsic oscillatory drive---e.g., central pattern generators (CPGs) \citep{marder2001central}---whose structure is unknown to the experimenter and whose state-dependence violates the standard input-state independence assumption of system identification.
