...
A Granger-inspired refinement step enforces biological constraints via projected gradient descent, achieving strong correlation with true connectivity and substantial improvement over established baselines, by pooling covariance statistics globally before inversion rather than fitting per session.
...
Iterating a trust-region step on $A$ with projection of $W = A \hat{\Sigma}_{x_t, x_t}^{-1}$ onto $\mathcal{C}$ yields a refined estimator that is the Frobenius-closest feasible point to the raw covariance-based solution.
...
After enforcing the no-autapse prior ($W_{ii} = 0$) and non-negativity ($W_{ij} \geq 0$) on the raw estimate, the Granger-inspired refinement further reduces error by an additional $\sim 4\%$ through its non-causality sparsity pattern.
...
The Granger-inspired refinement improves precision to $0.40$ while maintaining recall at $0.97$, eliminating spurious edges without sacrificing true connectivity.
...
Excluding the pathological $\sigma{=}0$ regime where all three estimators fail, the oracle penalty (ratio of oracle error to Granger-refined approximation error) is largest at low stimulation ($\sim2.2\times$ at $\sigma{=}0.1$, $\sim1.7\times$ at $\sigma = 0.25$--$0.5$) and narrows to $1.5$--$1.6\times$ for $\sigma \geq 1$.
...
Quantitatively, $\|E_2\|_F$ (input correlation) is ${\sim}3\times$ larger than $\|E_1\|_F$ (model mismatch), making CPG correlation the \emph{dominant} error source, not the linear approximation (Figure~\ref{fig:dynamics}H).
...
\textbf{(H)} Error decomposition: $\|E_1\|_F/N$ (model mismatch, $0.080$) vs.\ $\|E_2\|_F/N$ (CPG--state correlation, $0.225$) vs.\ total estimation error $\|\hat{W} - W\|_F/N$ ($0.087$). $E_1$ and $E_2$ are Frobenius norms of the \emph{individual error components} (Eq.~\ref{eq:error}); the total can be smaller because these components partially cancel in different matrix entries.  For this representative topology, $E_2$ is ${\sim}3\times$ larger than $E_1$ (median across topologies: ${\sim}3\times$), confirming that CPG correlation (not the linear approximation) is the dominant error source.
\textbf{(I)} Scatter plot of true off-diagonal weights $W_{ij}$ (horizontal) vs.\ Granger-refined estimates $\hat{W}_{ij}$ (vertical) for this representative topology. Pearson $r = 0.94$; dashed line: $y=x$. Median raw-estimator correlation across all 10 topologies: $r = 0.90$.}
\label{fig:dynamics}
\end{figure}
...
The Granger-inspired refinement's unique contribution is the lagged-covariance sparsity pattern ($W_{ij} = 0$ where contemporaneous covariance exceeds lagged; see Appendix~\ref{sec:app_granger_rule}), improving precision from $0.30$ to $0.40$ while maintaining high recall ($0.97$).
This indicates that the non-causality criterion effectively removes spurious edges without discarding true connections.
...
Third, error decomposition (E2) reveals that CPG--state correlation (${\sim}3\times$ larger than model mismatch) is the dominant bottleneck, suggesting that future gains will come from modeling intrinsic dynamics rather than refining the nonlinear approximation.
...
\begin{enumerate}
    ...
    \item \textbf{Necessary, not sufficient.} Eq.~\eqref{eq:lag1_expansion} sums contributions from all length-1 paths and includes $\mathrm{Cov}(b_{i,t}, x_{j,t})$; multi-path confounding or strong CPG--state coupling can cause the rule to miss a true edge. The rule is a conservative screening heuristic, not an identification theorem.
...
\end{enumerate}
Empirically (Section~\ref{sec:experiments}, E2) the rule improves precision from $0.30$ to $0.40$ while keeping recall at $0.97$---consistent with its interpretation as a conservative, precision-biased screen rather than a complete causal test.
