...
A Granger-causality refinement step enforces biological constraints via projected gradient descent, achieving strong correlation with true connectivity and substantial improvement over established baselines, by pooling covariance statistics globally before inversion rather than fitting per session.
Through systematic experiments on synthetic networks, motivated by small neural circuits where complete simultaneous recording still remains evasive, we characterize a fundamental \emph{control-estimation tradeoff}: stimulation aids identifiability but disrupts intrinsic dynamics, with the optimal level depending on measurement density.
We further show that the ``incorrect'' linear approximation outperforms the oracle estimator with known nonlinearity at all tested regimes, a concrete instance of James--Stein shrinkage.
Error decomposition reveals that drive from intrinsic circuit oscillators (central pattern generators, CPGs), not model mismatch, is the dominant bottleneck, identifying autonomous dynamics modeling as the primary target for further improvement.
\end{abstract}
...
\begin{itemize}
    ...
    \item \emph{Granger}: \emph{Estimate} refined with Granger non-causality zero constraints (Section~\ref{sec:granger}).
\end{itemize}
...
E2 asks what each component of the pipeline contributes and whether a simpler baseline could achieve the same recovery: can we recover well without the Granger refinement? Would a per-session VAR or Ridge-GLM be enough?
...
Taken together, the VAR and Ridge-GLM baselines (Appendix~\ref{sec:app_baselines}) confirm that the global accumulate-then-invert design---not the Granger refinement alone---is the essential ingredient: local per-session inversions, even when regularized, trail our global estimator by $36$--$58\%$.
...
The E6 experiment (Section~\ref{sec:experiments}, Figure~\ref{fig:oracle}) confirms this prediction empirically: the approximation matches or outperforms the oracle at all seven stimulation intensities tested, with the largest gap ($\sim2.7\times$) at $\sigma = 0$.
...
First, a fundamental \emph{control-estimation tradeoff} governs experimental design (E3, E7): stimulation is necessary for identifiability (zero stimulation fails at high measurement density), but excessive stimulation degrades the covariance structure.
...
Second, the ``incorrect'' linear approximation matches or outperforms the oracle estimator with known nonlinearity at \emph{all} tested regimes (E6), a concrete instance of the James--Stein phenomenon where implicit regularization reduces total risk.
...
Third, error decomposition (E2) reveals that CPG--state correlation (${\sim}2\times$ larger than model mismatch) is the dominant bottleneck, suggesting that future gains will come from modeling intrinsic dynamics rather than refining the nonlinear approximation.
...
Figure~\ref{fig:granger}H shows VAR at Frobenius error $0.136$ (worse than even the Spectral prior) and Ridge-GLM at $0.117$ ($36\%$ worse than our estimator at $0.086$), confirming that pooling covariance statistics \emph{before} the global inversion is the critical design choice: local per-session inversions---even when regularized---cannot recover information that only emerges from the joint structure of accumulated covariances.
