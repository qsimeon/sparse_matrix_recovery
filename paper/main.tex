...
By pooling covariance statistics globally before inversion rather than fitting per session, our estimator outperforms per-session vector autoregression and ridge-GLM baselines by $36$--$58\%$ in Frobenius error at the $N{=}15$ baseline; a Granger-inspired refinement step then enforces biological constraints (no autapses, non-negativity, lagged-covariance sparsity) via projected gradient descent.
Our headline result is counterintuitive: the ``incorrect'' linear approximation \emph{outperforms} the oracle estimator that knows the true nonlinearity, at every stimulation level we test---a concrete instance of James--Stein shrinkage where ill-conditioning of the oracle's compressed covariance overwhelms its bias advantage.
Through systematic experiments on synthetic networks, we further characterize a \emph{control-estimation tradeoff}: stimulation aids identifiability but disrupts intrinsic dynamics, with the optimal level depending on measurement density.
