...
code(
    ...
    "N = 12          # neurons\n"
    "T = 900         # timesteps per session\n"
    "K = 50          # sessions (network instances)\n"
    "NUM_CPGS = 4    # CPG nodes (33%)\n"
    "NUM_MEAS = 8    # measured nodes (66%)\n"
    "NUM_SENS = 4    # sensor nodes (33%)\n"
...
code(
    ...
    "sensor_mask = np.zeros(N, dtype=bool); sensor_mask[-NUM_SENS:] = True\n"
    "\n"
    "for t in range(T_demo):\n"
    "    cpg_drive2 = state_to_cpg(state2, cpg_net2)\n"
    "    b2 = cpg_mask * cpg_drive2 + sensor_mask * np.random.normal(0, STIM, N)\n"
...
md(
    ...
    "1. Generate $K=50$ sessions with shared $W$ but different measured/sensor subsets\n"
...
code(
    ...
    "    num_cpgs=NUM_CPGS, num_measured=NUM_MEAS, num_sensors=NUM_SENS,\n"
    "    fixed_sensors=False, stim_gain=STIM, nonlinearity=PHI,\n"
...
code(
    ...
    "        rep, SEED, K, T, N, NUM_CPGS, NUM_MEAS, NUM_SENS,\n"
...
md(
    ...
    "|---------|----------|---------------|\n"
    "| Covariance accumulation recovers $W$ | $r \\sim 0.90$ correlation | E1, E4 |\n"
